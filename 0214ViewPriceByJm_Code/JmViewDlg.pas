...
var
  ...
  i : Integer;
begin
  for i := 1 to ViewGrid.RowCount - 1 do   //for문도 1줄이면 beginend....
...
begin
  if ViewGrid <> nil then begin
    with ViewGrid do begin          //이부분 인자를row랑 인자 amdhoga 이런것들을 변수로 두고, 밖에서 받는걸로고치면 가독성 Up
      Cells[GC__HOGA, GR_MD4] := FormatFloat('0.##', (StrToInt(aMdhoga4)) / 100);  //여기서 차라리if hoga > 0 을 하는게 나아..화면에 보여주는건 퍼포먼스저하. 
